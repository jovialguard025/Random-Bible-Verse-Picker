...
bible_data = {
    ...
    "Psalms": {
        ...
        51: 21, 52: 9, 53: 6, 54: 7, 55: 23, 56: 13, 57: 11, 58: 11, 59: 17, 60: 12,
...
while True:
    books = random.choice(list(bible_data.keys()))
    chapter = random.randint(1,150)
    verse = random.randint(1,176)

    if chapter in bible_data[books]:
        max_verses_in_chapter = bible_data[books][chapter]
        if 1 <= verse <= max_verses_in_chapter:
            print(f"Bible Verse For Today: {books} {chapter}:{verse}")
            break
